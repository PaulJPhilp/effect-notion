...
import { loadSourcesConfig, applyDefaults, validateConfig } from "./config.js"
...
const loadFromConfig = (): ReadonlyArray<SourceConfig> => {
  const configPath = process.env.NOTION_SOURCES_CONFIG || "./sources.config.json"

  // Load and validate config file
  const configEffect = loadSourcesConfig(configPath).pipe(
    Effect.tap((config) => validateConfig(config)),
    Effect.catchAll((error) => {
      console.warn(`[Sources] Failed to load config from ${configPath}: ${error.message}`)
  ...
  if (sources.length > 0) {
    ...
    console.warn("[Sources] No sources configured. Check sources.config.json and environment variables.")
  }
