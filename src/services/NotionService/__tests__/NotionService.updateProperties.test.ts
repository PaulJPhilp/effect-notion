...
describe.skipIf(!process.env.NOTION_API_KEY || !NOTION_PAGE_ID)(
  ...
  () => {
    it("should retrieve article metadata", async () => {
      if (!NOTION_PAGE_ID) {
    ...
    it("should update article properties and return updated metadata", async () => {
      if (!NOTION_PAGE_ID) {
        throw new Error("Missing NOTION_PAGE_ID for integration test");
      }
      const exit = await Effect.runPromiseExit(
    ...
    it("should handle updating published date property", async () => {
      if (!NOTION_PAGE_ID) {
        throw new Error("Missing NOTION_PAGE_ID for integration test");
      }
      const exit = await Effect.runPromiseExit(
